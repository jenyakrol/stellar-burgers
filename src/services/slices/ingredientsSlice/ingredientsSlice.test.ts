...
describe('Слайс ингредиентов', () => {
  ...
  test('Проверка запроса ингредиентов с сервера', async () => {
    ...
    const getIngredientsPromise = store.dispatch(getIngredients());

    expect(store.getState().loading).toBe(true);

    await getIngredientsPromise;

    const { ingredients, loading } = store.getState();
  ...
  test('Проверка ошибки запроса ингредиентов', async () => {
    ...
    const { error, loading } = store.getState();
